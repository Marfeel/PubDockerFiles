...
RUN groupadd -g ${GID} ${GROUP} && \
    useradd -d ${USER_HOME} -m -r -u ${UID} -g ${GID} ${USER}

RUN apt-get update && \
    apt-get install -y locales tzdata && \
    locale-gen ${LOCALE_SETUP} && \
    echo "export LC_ALL=${LOCALE_SETUP}" >> ${USER_HOME}/.bashrc
...
RUN apt-get update && \
    ...
    rsync lastpass-cli && \
    echo 'source /usr/share/bash-completion/bash_completion' >> ${USER_HOME}/.bashrc
...
RUN curl -Ls https://releases.hashicorp.com/terraform/${TERRAFORM_VERSION}/terraform_${TERRAFORM_VERSION}_linux_amd64.zip | gunzip > /usr/local/bin/terraform && \
    ...
    mkdir -p ${USER_HOME}/.terraform.d/plugins && \
...
RUN curl -Ls https://awscli.amazonaws.com/awscli-exe-linux-x86_64.zip -o /tmp/awscliv2.zip && \
    ...
    /tmp/aws/install && \
    rm -fr /tmp/aw* && \
    echo "complete -C '/usr/local/bin/aws_completer' aws" >> ${USER_HOME}/.bashrc
...
RUN curl -Ls https://github.com/carlpett/terraform-provider-sops/releases/download/v${TERRAFORM_PROVIDER_SOPS_VERSION}/terraform-provider-sops_v${TERRAFORM_PROVIDER_SOPS_VERSION}_linux_amd64.zip \
    ...
    unzip -j /tmp/terraform-provider-sops.zip -d ${USER_HOME}/.terraform.d/plugins/ && \
    chmod +x ${USER_HOME}/.terraform.d/plugins/terraform-provider-sops_v${TERRAFORM_PROVIDER_SOPS_VERSION}
...
RUN curl -Ls https://github.com/github/hub/releases/download/v${HUB_VERSION}/hub-linux-amd64-${HUB_VERSION}.tgz | tar -zx -f - --one-top-level=/tmp/hub-linux --strip-components=1 && \
    bash /tmp/hub-linux/install && \
    rm -rf /tmp/hub-linux
...
RUN curl -L https://github.com/istio/istio/releases/download/${ISTIO_VERSION}/istio-${ISTIO_VERSION}-linux-amd64.tar.gz | tar -zx -C /usr/local/src && \
    echo "export PATH=$PATH:/usr/local/src/istio-${ISTIO_VERSION}/bin" >> ${USER_HOME}/.bashrc && \
...
RUN mkdir -p ${USER_HOME}/.ssh/ && \
    ...
    chown -R ${UID}:${GID} ${USER_HOME}/.terraform.d/ && \
